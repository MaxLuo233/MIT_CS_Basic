...
class CrossVerifier(object):
  ...
  def __init__(self, layer):
    ...
    self.index = RangeIndex() #改变存储的扫描坐标的数据结构，用AVL树提高搜索效率
...
if __name__ == '__main__':
    ...
    print(os.environ.get('TRACE'), os.environ.get('INDEX'))
    if os.environ.get('TRACE') == 'jsonp':
      verifier = TracedCrossVerifier(layer) # visualizer，不用管
